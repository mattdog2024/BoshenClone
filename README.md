...
### 2.5 多周期窗格测量（新功能）
* **`region_manager.py`**
  * **作用**：多周期窗格区域管理器。新版交易软件一个屏幕上同时显示 6 个周期（周线/日线/2小时/30分/5分/3分，2行3列网格）。本模块负责记住每个周期窗格在屏幕上的矩形位置、名字，并把这些区域定义持久化到 `regions.json`。
  * **想改这里**：如果您想调整区域的命名规则、命中判定逻辑，就在这里改。

* **多周期测量怎么用？**（操作流程）
  1. **框周期**：点工具栏第二行的「框周期」按钮，然后在屏幕上拖一个矩形把某个周期的 K 线图框起来，松手后给它起个名字（如“周线”）。重复操作框出您需要测量的几个周期窗格。
  2. **测量**：点「K线」按钮，再点该窗格里您要测量的最高点/最低点的那根 K 线，系统会自动识别整根 K 线的高低点并吸附测量。测量线会自动归属到它所在的周期窗格。
  3. **多周期同时锁定**：换到另一个窗格继续点「K线」测量即可。各个周期的测量结果会**同时显示并一直保持**，互不影响；每个窗格的横线只画在自己的矩形宽度内，不会横穿整个屏幕。
  4. **选周期**：点「选周期」按钮再点某个窗格，可把它设为“当前周期”（窗格边框会变成蓝色实线，左上角标注 `[当前]`）。
  5. **删本期**：删除“当前周期窗格”里的全部测量结果，方便重新测量。
  6. **删全部**：一键删除所有周期窗格里的全部测量结果。
  * 测量结果会自动保存到 `region_drawings.json`，关掉程序再打开还在。
