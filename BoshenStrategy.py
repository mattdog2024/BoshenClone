...
class BoshenStrategy(BaseStrategy):
    ...
    def _init_60min_down_measurement(self):
        ...
        从日线最高点(highest_since_ab)定位60分钟对应K线，
        用那根K线做向下测量。
        这是波神讲座的标准流程：日线定顶部→切60分钟对应K线→向下测量
        """
        ...
        # 日线最高点
        daily_high = self.highest_since_ab
        if daily_high <= 0:
            self.output('[DEBUG] 60分钟测量：日线最高点未确定，跳过')
            return

        # 在60分钟数据中找最接近日线最高价的K线
        # （日线最高点一定出现在某60分钟K线的high上）
        best_idx = -1
        best_diff = float('inf')
        search_start = max(0, n - 80)  # 最近80根（约两周）
        for i in range(n - 1, search_start - 1, -1):
            diff = abs(self._60min_highs[i] - daily_high)
            if diff < best_diff:
                best_diff = diff
                best_idx = i
            if diff < 0.5:  # 完全匹配就停
        ...
        if best_idx < 0:
            self.output('[DEBUG] 60分钟测量：未找到匹配日线高点的K线')
            return

        k_high = float(self._60min_highs[best_idx])
        k_low = float(self._60min_lows[best_idx])
        k_open = float(self._60min_opens[best_idx])
        k_close = float(self._60min_closes[best_idx])

        # 调试：输出找到的K线详情
        self.output(
            f'[DEBUG] 60分钟测量基准K线(对应日线高={daily_high:.2f}): '
            f'high={k_high:.2f}, low={k_low:.2f}, '
        ...
        if (is_pullback or is_rebound) and pullback_end_confirmed and pullback_bottom > 0:
            ...
            if m60 and m60.get('start') is not None:
                ...
                if m60_line_num >= 7:
                    ...
                    # 日线回调结束后，60分钟在1-4线：不要做空！等60分钟跌到7-8线再找做多机会
                    lines.append("▶ 60分钟还在1-4线区域，尚未到达反转区（7-8线）")
                    lines.append("  ⚠️ 不要做空！日线大方向向上，等60分钟跌到7-8线再找做多机会")
                    lines.append("  ✅ 耐心等待60分钟下跌到7-8线附近，出现向上形态后再入场做多")
                    if pattern_60:
                        lines.append(f"  60分钟形态: ❗ {pattern_60}")
                        if '看多' in pattern_60 or '阳包阴' in pattern_60 or '止跌' in pattern_60 or '锤子' in pattern_60:
                            lines.append("  → ⚠️ 60分钟出现向上形态，但位置偏高，谨慎入场，等更好的位置")
                    else:
                        lines.append("  60分钟形态：等待60分钟继续下跌到7-8线区域...")
